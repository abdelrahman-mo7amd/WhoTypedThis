...
export const Game = {
    async createRoom() {
        ...
        if(!name) return window.UI?.toast('Enter your name 😅', 'bad');
        ...
        const roomData = {
            ...
            timerSec: timer,
            phase:'lobby',
            players: [{id: myId, name, avatar, color:COLORS[0], score: 0, uid: user?.uid || null }],
            prompts: [],
            currentRound: 0,
            promptsDone: [],
            guessesDone: [],
        ...
        try {
            ...
            window.UI?.toast("Failed to create room - check connection", 'bad');
        }
    },
    ...
    async joinRoom(code) {
        ...
        room.players.push({id: myId, name, avatar, color: COLORS[idx % COLORS.length], score:0, uid: user?.uid || null} ); 
        try{
    ...
    async _renderLobby(room) {
        ...
        room.players.forEach((p,i) => {
            ...
            el.innerHTML = `
                ...
                <div class="player-name">${esc(p.name)}${isMe ? ' <span style="color:var(--trans);font-size:12px">(you)</span>' : ''}</div>
        ...
        if (startBtn && S.isHost) {
            ...
            startBtn.textContent = can ? 'Start Game 🚀': "Need 2+ players to start";
        }
        ...
        if (S.isHost && profile?.friends.length) {
            ...
            const friendIds = profile.friends(0,8);
            ...
            if (invList) {
                invList.innerHTML = friendProfiles.filter(Boolean).map(f => `
                    <button class="hint-pill" onclick="Game.inviteFriend('${f.uid}')" title="Invite ${esc(f.displayName)}">
    ...
    async startGame() {
        ...
        if(!room||room.players.length <3) return window.UI?.toast('Need at least 3 players', 'bad');

        const players = room.players;
        const prompts = [];
        for (let r = 0; r < room.players; r++) {
        ...
        await DB.updateRoom(S.roomCode, {phase: "prompting", prompts, currentRound:0, promptsDone: [], guessesDone:[], roundGuesses: {}, roundHistory :[]});
        window.Audio?.start();
    },
    ...
    _enterPrompt(room) {
        ...
        if (myPrompt) {
            ...
            document.getElementById('prompt-target-name').textContent = about ? `${about.avatar} ${about.name}` : '?';
        }
    ...
    _renderPromptWaiting(room) {
        ...
        waitList.innerHTML = room.players.map(p=> {
            ...
            return `
                <div class="waiting-item ${done? 'done' : ''}">
        ...
        if (S.isHost && room.promptsDone?.length >= room.players.length){
            DB.updateRoom(S.roomCode, {phase: "guessing", currentRound: 0, guessesDone: [] });
        }
    },
    ...
    _renderGuessRound(room) {
        ...
        document.getElementsById('guess-round-num').textContent = round + 1;
        ...
        if (guessGrid) {
            guessGrid.innerHTML = room.players.map(p => `
                    <button class="guess-btn" onclick="Game._makeGuess('${p.id}', '${prompt.authorId}')" ${S.guessLocked ? 'disabled' : ''}>
                        <span class="gavatar">${p.avatar}</span>
                        <span>${esc(p.name)}${p.id===S.myId ? ' (you)': ''}</span>
                    </button>
                `).join('');
        }
    ...
    async _makeGuess(guessedId, actualAuthorId) {
        ...
        window.Audio?.[correct? 'correct':'wrong']();
        ...
        const author = (await DB.getRoom(S.roomCode))?.players?.find(p=> p.id === actualAuthorId);
        const area = document.getElementById('reveal-card');
        if (area) {
            area.innerHTML = `
                <div class="reveal-card ${correct ? 'correct-reveal' : 'wrong-reveal'}">
                    <div class="reveal-verdict">${correct ? '✓ CORRECT!' : '✗ WRONG'}</div>
                    <div class="reveal-sub">Written by ${author?.avatar || ''} ${esc(author?.name || '?')}
        ...
        if (reactionBar) {
            ...
            reactionBar.innerHTML = ['😂','🔥','💀','👀','🫡','🤣'].map(e => `<button class="reaction-btn" onclick="Game._sendReaction('${e}', this)">${e}</button>`).join('');
        }

        const room = await DB.getRoom(S.roomCode);
        if (!room) return;

        const roundGuesses = {...(room.roundGuesses || {}) };
        roundGuesses[S.myId] = { guessedId, correct, pts};

        const guessesDone = [...AVATARS(room.guessesDone || [])];
        if (!guessesDone.includes(S.myId)) guessesDone.push(S.myId);

        const players = room.players.map (p=> p.id === S.myId ? {...p, score: (p.score || 0) + pts} : p);
        ...
        if (S.isHost) {
            if (nextBtn) {
                ...
                nextBtn.textContent = (room.currentRound + 1) < room.prompts.length ? 'Next Round →' : 'See Results 🏆';
            }
        } else {
            if (nextWrap) nextWrap.innerHTML = '<div class="notice">⏳ Waiting for host to advance...</div>';
        }
    },
    ...
    async nextRound() {
        ...
        if (next >= (room?.prompts?.length || 0)) {
            ...
            await DB.updateRoom(S.roomCode, {currentRound: next, roundGuesses: {}, guessesDone: []});
        }
    },
    ...
    _startTimer(sec, authorId) {
        ...
        if (bar) {
            bar.classList.remove('timer-hurry');
        }
    ...
    async _enterFinal(room) {
        ...
        sorted.forEach((p,i) => {
            ...
            el.innerHTML = `
                ...
                <div class="score-name">${esc(p.name)}${p.id === S.myId ? ' (you)' : ''}</div>
        ...
        const recap = document.getElementById('recep-board');
        recap.innerHTML = room.prompts.map(pr => {
        ...
        if (uid && me) {
            const correct = Object.values(room.roundGuesses || {})
            .filter((g) => g?.correct).length;
    ...
    async playAgain() {
        if (S.isHost) {
            await DB.updateRoom(S.roomCode, {
                phase: 'lobby', prompts: [], promptsDone: [], currentRound: 0,
                guessesDone: [], roundGuesses: {}, roundHistory: [],
                players : (await DB.getRoom(S.roomCode))?.players.map(p => ({...p, score: 0})) || [],
            });
            Game._enterLobby();
        } else {
            window.UI?.toast("Waiting for host to restart...", 'bad');
        }
    },
    ...
    _spawnConfetti(area){
        ...
        for (let i = 0; i < 16; i++) {
            ...
            d.style.cssText = `left: ${10 + Math.random() * 80}%; top: 0; background: ${colors[i % colors.length]}; animation-delay: ${Math.random() * 0.4}s`;
            area.appendChild(d);
